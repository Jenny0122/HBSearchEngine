...
public class WNSearch {
	 ...
	 */
	public WNSearch(String searcherId, boolean isDebug, boolean isUidSrch, String[] collections,
			String[] searchFields) {
	 ...
	 */
	public WNSearch(String searcherId, boolean isDebug, boolean isUidSrch, String[] collections, String[] searchFields,
			int resultType) {
		this.common = new WNCommon();
		this.wncol = new WNCollection();
		this.collections = collections;
	 ...
	 * @param resultType 검색결과를 반환받고자 하는 데이터 형태
	 */
	public WNSearch(boolean isDebug, boolean isUidSrch, String[] collections, String[] searchFields, int resultType) {
		this.common = new WNCommon();
		this.wncol = new WNCollection();
		this.collections = collections;
		this.searchFields = searchFields;
	 ...
	 * @param resultType 검색결과를 반환받고자 하는 데이터 형태
	 */
	public WNSearch(boolean isDebug, boolean isUidSrch, String[] collections, String[] mergeCollections,
	 ...
	 * @param searchFields 검색하고자 하는 검색필드들을 Array로 지정한다.
	 */
	public WNSearch(boolean isDebug, boolean isUidSrch, String[] collections, String[] searchFields, String apprType) {
		this.common = new WNCommon();
		this.wncol = new WNCollection(apprType);
		this.collections = collections;
		this.searchFields = searchFields;
		this.sb = new StringBuffer();
		this.warningMsg = new StringBuffer();
	...
	public int search(String query, boolean isRealQuery, int useOrResult) {
		return search(query, isRealQuery, null, HI_SUM_ONON, useOrResult, CONNECTION_CLOSE, false, "appr");
	}
	...
	public int search(String query, boolean isRealQuery, int connectionOpt, boolean useSuggestedQuery) {
		...
		return search(query, isRealQuery, null, HI_SUM_ONON, COMMON_OR_WHEN_NORESULT_OFF, connectionOpt,
				useSuggestedQuery, "appr");
	}
	 ...
	 */
	public int search(String query, boolean isRealQuery, int connectionOpt, boolean useSuggestedQuery,
			String[] userInfo, String apprType) {
		this.connectionOpt = connectionOpt;
		return search(query, isRealQuery, userInfo, HI_SUM_ONON, COMMON_OR_WHEN_NORESULT_OFF, connectionOpt,
				useSuggestedQuery, apprType);
	}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int search(String query, boolean isRealQuery, int useOrResult, int connectionOpt,
			boolean useSuggestedQuery) {
		this.connectionOpt = connectionOpt;
		return search(query, isRealQuery, null, HI_SUM_ONON, useOrResult, connectionOpt, useSuggestedQuery, "appr");
	}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int search(String query, boolean isRealQuery, int useOrResult, int connectionOpt, int hiSum,
			boolean useSuggestedQuery) {
		this.connectionOpt = connectionOpt;
		return search(query, isRealQuery, null, hiSum, useOrResult, connectionOpt, useSuggestedQuery, "appr");
	}
	 ...
	 * @param connectionOpt 검색기와 연결을 유지할 것인지 끊을 것인지 여부를 결정
	 * @param useSuggestedQuery speller 결과를 사용할 것인지 여부를 결정
	 * @param apprType 완료문서 이관문서 구분용
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	private int search(String query, boolean isRealQuery, String[] userInfo, int hiSum, int useOrResult,
			int connectionOpt, boolean useSuggestedQuery, String apprType) {
		this.connectionOpt = connectionOpt;
		this.hiSum = hiSum;
		int ret = 0;
		if (!isUidSrch) {
		...
		if (collections != null) {
			for (int i = 0; i < collections.length; i++) {
				...
				if (collections[i].equals("apprMig")) {
					idx = getCollIdx("appr");
				}
				//컬렉션 index 결과 비교
				if (idx < 0) {
					appendWarnings("[WARNING] [w3AddCollection] [search] [" + collections[i]
							+ "] Collection name is not exist");
					break;
				}

				//검색 컬렉션 설정
				ret = setCollectionBasicInfo(idx, apprType);

				//검색 필드 설정
				if (!isUidSrch) {
					ret = setSearchFieldInfo(idx, apprType);
					//날짜 범위 검색
					ret = setDateRange(idx, apprType);
					//Rank 범위 검색
					ret = setRankRange(idx, apprType);
					//PrefixQuery 조건 검색
					ret = setPrefixQuery(idx, apprType);
					//CollectionQuery 검색
					ret = setCollectionQuery(idx, apprType);
					//카테고리 부스팅 적용
					ret = setBoostCategory(idx, apprType);
					//카테고리 그룹바이
					ret = addCategoryGroupBy(idx, apprType);
					//카테고리 검색
					ret = addCategoryQuery(idx, apprType);
					//FilterQuery 조건 검색
					ret = setFilterQuery(idx, apprType);
					//권한 검색 조건 
					ret = setAuthorityQuery(idx, apprType);
					//속성 그룹 검색 조건 
					ret = setPropertyGroup(idx, apprType);
					//그룹바이 조건검색
					ret = setGroupBy(idx, apprType);
					//멀티그룹바이 조건검색
					ret = setMultiGroupBy(idx, apprType);
					// DEDUP 조건검색
					ret = setDuplicateDetectionCriterionField(idx, apprType);
				} else {
					//UID검색						
					ret = setUid(idx, query, searcherId);
				}
				//결과 필드 설정
				ret = setResultFieldInfo(idx, apprType);
			}
		...
		if (mergeCollections != null) {
			for (int n = 0; n < mergeCollections.length; n++) {
				...
				if (idx < 0) {
					...
					appendWarnings("[WARNING] [w3AddMergeCollection] [search] [" + mergeCollections[n]
							+ "] Merge Collection name is not exist");
					break;
				}
				//가상 collection 설정
	...
	public int setCollectionBasicInfo(int idx, String apprType) {
		...
		if (analyzer.length < 5 || pageInfo.length < 2 || rankingOption.length < 3) {
			appendWarnings("[WARNING] [setCollectionBasicInfo] [" + THIS_COLLECTIONS[idx] + "] Analyzer : '"
					+ wncol.COLLECTION_INFO[idx][ANALYZER] + "', PageInfo : '" + wncol.COLLECTION_INFO[idx][PAGE_INFO]
					+ "', RankingOption : '" + wncol.COLLECTION_INFO[idx][RANKING_OPTION] + "' error");
			return -1;
		}
		...
		if (checkKeywordAnalyzer(THIS_COLLECTIONS[idx], useKma, isCase, useOriginal, useSynonym,
				useDuplicateDetection) == -1) {
			return -1;
		}

		ret = addCollection(idx, useKma, isCase, useOriginal, useSynonym, useDuplicateDetection, apprType);

		if (checkSetPageInfo(THIS_COLLECTIONS[idx], this.hiSum, Integer.parseInt(pageInfo[0]),
				Integer.parseInt(pageInfo[1])) == -1) {
			return -1;
		}
		ret = common.setPageInfo(THIS_COLLECTIONS[idx], this.hiSum, Integer.parseInt(pageInfo[0]),
				Integer.parseInt(pageInfo[1]));

		ret = common.setRanking(THIS_COLLECTIONS[idx], rankingOption[0], rankingOption[1],
				parseInt(rankingOption[2], 100));
		...
		for (String sortField : sortFieldInfo.split(",")) {
			...
			if (checkSortFieldName(THIS_COLLECTIONS[idx], tokens[0], tokens[1]) != 0)
				return -1;
		}
		ret = common.setSortField(THIS_COLLECTIONS[idx], sortFieldInfo);
		return ret;
	}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int addCollection(int idx, int useKma, int isCase, int useOriginal, int useSynonym,
			int useDuplicateDetection, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		String indexName = THIS_COLLECTIONS[idx];
		...
		if (indexName.equals(colName)) {
			...
			if (useOriginal == -1 && useSynonym == -1) {
				...
				ret = common.addAliasCollection(indexName, colName, useKma, isCase, useOriginal, useSynonym,
	...
	public boolean setCollectionInfoValue(String collName, int target, String value) {
		...
		if (collName.equals("apprMig")) {
			idx = getCollIdx("appr");
		}
		if (idx == -1) {
			appendWarnings("[WARNING] [w3AddCollection] [setCollectionInfoValue] [" + collName
					+ "] Collection name is not exist");
			return false;
		}
	...
	public boolean setMergeCollectionInfoValue(String collName, int target, String value) {
		...
		if (idx == -1) {
			appendWarnings("[WARNING] [w3AddMergeCollection] [setMergeCollectionInfoValue] [" + collName
					+ "] Collection name is not exist");
			return false;
		}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int setSearchFieldInfo(int idx, String apprType) {
		...
		if (searchFieldList.length < 1) {
			appendWarnings("[WARNING] [w3AddSearchField] [setAllSearchFieldInfo] [" + THIS_COLLECTIONS[idx]
					+ "] SEARCH_FIELD should be defined.");
			return -1;
		}
		int i = checkFieldName("[w3AddSearchField]", THIS_COLLECTIONS[idx], searchFieldList, SEARCH_FIELD);
		if (i != 0) {
			appendWarnings("[WARNING] [w3AddSearchField] [setAllSearchFieldInfo] [" + THIS_COLLECTIONS[idx] + "] '"
					+ searchFieldList[i - 1] + "' is not exist.");
			return -1;
		}
		ret = common.setSearchField(THIS_COLLECTIONS[idx], searchFields);
		return ret;
	}
	 ...
	 * @return  성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int setCollectionQuery(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][COLLECTION_QUERY].equals("")) {
			if (isDebug) {
				checkCollectionQueryFieldName("[w3SetCollectionQuery] [setCollectionQuery]", THIS_COLLECTIONS[idx],
						wncol.COLLECTION_INFO[idx][COLLECTION_QUERY]);
			}
			ret = common.setCollectionQuery(THIS_COLLECTIONS[idx], wncol.COLLECTION_INFO[idx][COLLECTION_QUERY]);
		}
		return ret;
	}
	...
	public int setDateRange(int idx, String apprType) {
		...
		if (!wncol.COLLECTION_INFO[idx][DATE_RANGE].equals("")) {
			...
			if (dateFields.length < 2) {
				appendWarnings("[WARNING] [w3SetDateRange] [setDateRange] [" + THIS_COLLECTIONS[idx]
						+ "] DATE_RANGE : '" + wncol.COLLECTION_INFO[idx][DATE_RANGE] + "' error.");
				return -1;
			}
			ret = common.setDateRange(THIS_COLLECTIONS[idx], dateFields[0], dateFields[1], dateFields[2]);
		}
		return ret;
	}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int setRankRange(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][RANK_RANGE].equals("")) {
			...
			if (rankFields.length < 2) {
				appendWarnings("[WARNING] [w3SetRankRange] [setRankRange] [" + THIS_COLLECTIONS[idx]
						+ "] RANK_RANGE : '" + wncol.COLLECTION_INFO[idx][RANK_RANGE] + "' error.");
				return -1;
			}
			int min = parseInt(rankFields[0], 0);
			int max = parseInt(rankFields[1], 100);
			ret = common.setRankRange(THIS_COLLECTIONS[idx], min, max);
		}
		return ret;
	}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int setPrefixQuery(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][EXQUERY_FIELD].equals("")) {
			if (isDebug) {
				checkExFieldName("[w3AddExQuery] [setExQuery]", THIS_COLLECTIONS[idx],
						wncol.COLLECTION_INFO[idx][EXQUERY_FIELD]);
			}
			ret = common.setPrefixQuery(THIS_COLLECTIONS[idx], wncol.COLLECTION_INFO[idx][EXQUERY_FIELD]);
		}
		return ret;
	}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int setBoostCategory(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][CATEGORY_BOOST].equals("")) {
			...
			if (dataFields.length == 3) {
				ret = common.setBoostCategory(THIS_COLLECTIONS[idx], dataFields[0], dataFields[1], dataFields[2]);
			}
		}
		return ret;
	}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int addCategoryGroupBy(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][CATEGORY_GROUPBY].equals("")) {
			...
			common.addCategoryGroupBy(THIS_COLLECTIONS[idx], categoryGroup);
		}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int addCategoryQuery(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][CATEGORY_QUERY].equals("")) {
			...
			ret = common.addCategoryQuery(THIS_COLLECTIONS[idx], categoryQuery);
		}
	...
	* @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	*/
	public int setAuthorityQuery(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][AUTH_QUERY].equals("")) {
			...
			if (dataFields.length == 4) {
				ret = common.setAuthorityQuery(THIS_COLLECTIONS[idx], dataFields[0], dataFields[1], dataFields[2],
						dataFields[3]);
			}
	...
	* @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	*/
	public int setPropertyGroup(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][PROPERTY_GROUP].equals("")) {
			...
			if (dataFields.length == 4) {
				ret = common.setPropertyGroup(THIS_COLLECTIONS[idx], dataFields[0], parseInt(dataFields[1], 0),
	...
	*/
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int setFilterQuery(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][FILTER_OPERATION].equals("")) {
			if (isDebug) {
				checkFilterFieldName("[w3SetFilterQuery] [setFilterQuery]", THIS_COLLECTIONS[idx],
						wncol.COLLECTION_INFO[idx][FILTER_OPERATION]);
			}
			ret = common.setFilterQuery(THIS_COLLECTIONS[idx], wncol.COLLECTION_INFO[idx][FILTER_OPERATION]);
		}
		return ret;
	}

	public int setMultiGroupBy(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][MULTI_GROUP_BY].equals("")) {
			...
			//그룹전달인자 확인
			ret = common.setMultiGroupBy(THIS_COLLECTIONS[idx], field);
		}
		return ret;
	}
	...
	*/
	public String getMultiGroupByResult(String collectionName, String fieldName) {
		...
		chk = checkValidDocumentField(collectionName, fieldName, RESULT_FIELD);
		if (chk == -1) {
			appendWarnings("[WARNING] [w3AddMultiGroupBy] [getMultiGroupByResult] [" + collectionName
					+ "] RESULT_FIELD should be defined.");
			return null;
		}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int setGroupBy(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][GROUP_BY].equals("")) {
			...
			if (isDebug) {
				...
				if (checkValidField(THIS_COLLECTIONS[idx], groupby[0], FAST_ACCESS) == -1) {
					appendWarnings("[WARNING] [w3SetGroupBy] [setGroupBy] [" + THIS_COLLECTIONS[idx] + "] " + groupby[0]
							+ " is not fastaccess field.");
				}
			}
			//그룹전달인자 확인
			if (groupby.length == 2) {
				//그룹화 설정
				ret = common.setGroupBy(THIS_COLLECTIONS[idx], groupby[0], parseInt(groupby[1], 0));
				//그룹 정렬 전달 인자 확인
				ret = addSortFieldInGroup(idx);
			} else {
				appendWarnings("[WARNING] [w3SetGroupBy] [setGroupBy] [" + THIS_COLLECTIONS[idx] + "] "
						+ wncol.COLLECTION_INFO[idx][GROUP_BY] + "  Check variable format and value.");
			}
		}
		return ret;
	}
	 ...
	 */
	public int setDuplicateDetectionCriterionField(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (!wncol.COLLECTION_INFO[idx][DEDUP_SORT_FIELD].equals("")) {
			...
			ret = common.setDuplicateDetectionCriterionField(THIS_COLLECTIONS[idx], sortValue);
		}
		return ret;
	}
	...
	public String getFieldInGroup(String collectionName, String fieldName, int groupIndex, int docIndex) {
		...
		chk = checkValidDocumentField(collectionName, fieldName, RESULT_FIELD);
		if (chk == -1) {
			appendWarnings("[WARNING] [w3AddDocumentField] [setResultFieldInfo] [" + collectionName
					+ "] RESULT_FIELD should be defined.");
			return null;
		}
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int setResultFieldInfo(int idx, String apprType) {
		int ret = 0;
		String[] THIS_COLLECTIONS = null;
		if (apprType.equals("mig")) {
			THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		if (resultFieldList.length < 1) {
			appendWarnings("[WARNING] [w3AddDocumentField] [setResultFieldInfo] [" + THIS_COLLECTIONS[idx]
					+ "] RESULT_FIELD should be defined.");
			return -1;
		}
		int i = checkFieldName("[w3AddDocumentField]", THIS_COLLECTIONS[idx], resultFieldList, RESULT_FIELD);
		if (i != 0) {
			appendWarnings("[WARNING] [w3AddDocumentField] [setResultFieldInfo] [" + THIS_COLLECTIONS[idx] + "] '"
					+ resultFieldList[i - 1] + "' is not exist.");
			return -1;
		}
		ret = common.setResultField(THIS_COLLECTIONS[idx], resultFields);
		return ret;
	}
	...
	*/
	public int setMergeCollectionBasicnfo(int idx) {
		...
		if (pageInfo.length < 2) {
			appendWarnings("[WARNING] [setMergeCollectionBasicnfo] [PageInfo : '"
					+ wncol.MERGE_COLLECTION_INFO[idx][MERGE_PAGE_INFO] + "' error");
			return -1;
		}
	...
	public int addMergeDocumentField(int idx) {
		...
		for (int k = 0; k < collectionLength; k++) {
			for (int n = 0; n < temp.length; n++) {
				if (k == n)
					documentFields[k] = split(temp[n], ",");
				if (mergeFieldLength != documentFields[k].length) {
					appendWarnings(
							"[Error] [addMergeDocumentField] [merge fields, collection document and different.]");
					return -1;
				}
			}
		}
			...
			for (int k = 0; k < collectionLength; k++) {
				for (int n = 0; n < temp.length; n++) {
					...
					if (mergeLength != documentFields[k].length) {
						appendWarnings(
								"[Error] [addMergeMultiGroupBy] [merge multi-group-by, collection multi-group-by and different.]");
						return -1;
					}
				}
			}

			ret = common.addMergeMultiGroupByField(mergeCollectionName, mergeMultiGroupByFields, collectionNames,
					documentFields);
		}
		return ret;
	}
			...
			for (int k = 0; k < collectionLength; k++) {
				for (int n = 0; n < temp.length; n++) {
					...
					if (mergeCategoryLength != documentFields[k].length) {
						appendWarnings(
								"[Error] [addMergeCategoryGroupByField] [merge category-group-by, collection category-group-by and different.]");
						return -1;
					}
				}
			}

			ret = common.addMergeCategoryGroupByField(mergeCollectionName, mergeCategoryGroupByFields, collectionNames,
					documentFields);
		}
		return ret;
	}
	...
	*/
	...
	public int getCollIdx(String collName) {
		for (int i = 0; i < COLLECTIONS.length; i++) {
			if (COLLECTIONS[i].trim().equals(collName.trim()))
				return i;
		}
		return -1;
	}
	...
	public int getMergeCollIdx(String collName) {
		for (int i = 0; i < MERGE_COLLECTIONS.length; i++) {
			if (MERGE_COLLECTIONS[i].trim().equals(collName.trim()))
				return i;
		}
		return -1;
	}
	...
	*/
	 ...
	 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
	 */
	public int checkKeywordAnalyzer(String collectionName, int useKma, int isCase, int useOriginal, int useSynonym,
			int useDuplicateDetection) {
		int chk = 0;
		if (useKma != 0 && useKma != 1 && useKma != 2) {
			appendWarnings("[WARNING] [w3SetQueryAnalyzer] [checkKeywordAnalyzer] [" + collectionName + "] '" + useKma
					+ "' is not kma option");
			return -1;
		}
		if (isCase != 0 && isCase != 1) {
			appendWarnings("[WARNING] [w3SetQueryAnalyzer] [checkKeywordAnalyzer] [" + collectionName + "] '" + isCase
					+ "' is not case option");
			return -1;
		}
		if (useOriginal != 0 && useOriginal != 1) {
			appendWarnings("[WARNING] [w3SetQueryAnalyzer] [checkKeywordAnalyzer] [" + collectionName + "] '"
					+ useOriginal + "' is not use original option");
			return -1;
		}
		if (useSynonym != 0 && useSynonym != 1) {
			appendWarnings("[WARNING] [w3SetQueryAnalyzer] [checkKeywordAnalyzer] [" + collectionName + "] '"
					+ useSynonym + "' is not use synonym option");
			return -1;
		}
		if (useDuplicateDetection != 0 && useDuplicateDetection != 1) {
			appendWarnings("[WARNING] [w3SetQueryAnalyzer] [checkKeywordAnalyzer] [" + collectionName + "] '"
					+ useDuplicateDetection + "' is not use duplicate detection option");
			return -1;
		}
	...
	public int checkValidCollection(String msg, String collectionName) {
		...
		if (collectionName.equals("apprMig")) {
			chk = getCollIdx("appr");
		}
	...
	public int checkExFieldName(String msg, String collectionName, String value) {
		...
		for (int i = 0; i < fieldName.length; i++) {
			...
			if (checkValidField(collectionName, fieldName[i], PREFIX_FIELD) == -1) {
				appendWarnings(
						"[WARNING] " + msg + "[" + collectionName + "] " + fieldName[i] + " is not prefix field.");
				chk = -1;
			}
		}
		return chk;
		...
		for (int i = 0; i < fieldName.length; i++) {
			...
			if (fieldName[i].indexOf("<", 0) != -1) {
				...
				if (checkValidField(collectionName, fieldName[i], FAST_ACCESS) == -1) {
					appendWarnings("[WARNING] " + msg + " [" + collectionName + "] " + fieldName[i]
							+ " is not fastaccess field.");
					chk = -1;
				}
			}
	...
	public int checkSetPageInfo(String collectionName, int highlight, int startPos, int resultCnt) {
		...
		if (highlight < 0 || highlight > 3 || startPos < 0 || resultCnt < 0) {
			appendWarnings("[WARNING] [] [checkSetPageInfo] [" + collectionName + "] Highlight: '" + highlight
					+ "', StartPosition: '" + startPos + "', ResultCount: '" + resultCnt + "' is invalid value");
			return -1;
		}
		return chk;
	}
	...
	public int checkSortFieldName(String collectionName, String sortField, String sortOrder) {
		if (sortField.toUpperCase().equals("DATE")) {
			if (sortOrder.equals("DESC") == false && sortOrder.equals("ASC") == false) {
				...
				appendWarnings("[ERROR] [w3AddSortField] [checkSortFieldName] [" + collectionName
						+ "] Date sortOrder is '" + sortOrder + "'. Only DESC or ASC is allowed.");
				return -1;
			}
			...
			if (sortOrder.equals("DESC") == false) {
				...
				appendWarnings("[ERROR] [w3AddSortField] [checkSortFieldName] [" + collectionName
						+ "] RANK sortOrder is '" + sortOrder + "' Only DESC is allowed.");
				return -1;
			}
			...
			if (sortOrder.equals("DESC") == false && sortOrder.equals("ASC") == false) {
				...
				appendWarnings("[ERROR] [w3AddSortField] [checkSortFieldName] [" + collectionName
						+ "] WEIGHT sortOrder is '" + sortOrder + "' Only DESC or ASC is allowed");
				return -1;
			}
			...
			if (sortOrder.equals("DESC") == false && sortOrder.equals("ASC") == false) {
				...
				appendWarnings("[ERROR] [w3AddSortField] [checkSortFieldName] [" + collectionName
						+ "] UID sortOrder is '" + sortOrder + "' Only DESC or ASC is allowed");
				return -1;
			}
		} else {
			// 정렬 필드 검사
			if (checkValidDocumentField(collectionName, sortField, RESULT_FIELD) == -1) {
				appendWarnings("[ERROR] [w3AddSortField] [checkSortFieldName] [" + collectionName + "] '" + sortField
						+ "' is not exist.");
				return -1;
			}
			if (checkValidField(collectionName, sortField, FAST_ACCESS) == -1) {
				// fastaccess가 지정되지 않은 필드에서 속도 느릴수 있음.
				appendWarnings("[WARNING] [w3AddSortField] [checkSortFieldName] [" + collectionName + "] " + sortField
						+ " is not fastaccess field.");
			}
		}
		return 0;
	...
	public void debugMsg(boolean isRealQuery, String query) {
		...
		sb.append("<br>[w3ConnectServer] [IP] " + SEARCH_IP + " [PORT] " + SEARCH_PORT + " [TIMEOUT]"
				+ CONNECTION_TIMEOUT);
		if (!isUidSrch) {
		...
		if (mergeCollections != null) {
			for (int i = 0; i < mergeCollections.length; i++) {
				...
				if (content != null && !content.equals("") && mappingColl != null && !mappingColl.equals("")) {
					sb.append("<br>[w3AddMergeCollection] <b>" + content + "</b>, [Collection Name]<b>" + mappingColl
							+ "</b>");
				}
				...
				if (content != null && !content.equals("") && mappingDocs != null && !mappingDocs.equals("")) {
					sb.append("<br>[w3AddMergeDocumentField] " + content + ", [Collection Document Fields]"
							+ mappingDocs);
				}
				...
				if (content != null && !content.equals("") && mappingMulti != null && !mappingMulti.equals("")) {
					sb.append("<br>[w3AddMergeMultiGroupByField] " + content + ", [Collection Multi-Groupby Fields]"
							+ mappingMulti);
				}
				...
				if (content != null && !content.equals("") && mappingCategory != null && !mappingCategory.equals("")) {
					sb.append("<br>[w3AddMergeCategoryGroupByField] " + content
							+ ", [Collection Category Groupby Fields]" + mappingCategory);
				}
			}
	...
	public String getManagerOperationTag(String colName, int idx, String docid) {
		...
		if (!this.managerURL.equals("")) {
			return "<input type='checkbox' name='manager_idx' id='" + uniid
					+ "'  value='checkbox' onclick=\"javascript:manager_addDocIdForDelete('" + colName + "', '" + docid
					+ "', '" + uniid + "')\" />" + docid + "&nbsp";
		} else {
			return "";
		}
	...
	public String getManagerActionTag(String transferURL) {
		if (!this.managerURL.equals(""))
			return "<input type='button'  value='Delete' onclick=\"javascript:manager_runDelete('" + transferURL
					+ "', '" + this.managerURL + "', '" + SEARCH_IP + "', " + SEARCH_PORT + ")\" />";
		else
			return "";
	}
	...
	public String getPageLinks(int startCount, int totalCount, int viewListCount, int bundleCount) {
		...
		int lastPageNum = (totalCount / viewListCount) * viewListCount;
		if (totalCount % viewListCount == 0) {
			lastPageNum -= viewListCount;
		}

		if (wnanchor.getFirstPage() != -1) {
			sbRet.append(
					"<a class=\"first\" href=\"#none\" onClick=\"javascript:doPaging('0');\" title=\"First Page\"><img src=\"images/btn_first.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
			sbRet.append("<a class=\"prev\" href=\"#none\" onClick=\"javascript:doPaging('" + wnanchor.getBundleBefore()
					+ "');\" title=\"Previous Page\"><img src=\"images/btn_prev.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
		} else {
			sbRet.append(
					"<a class=\"first\" href=\"#none\" onClick=\"javascript:doPaging('0');\" title=\"First Page\"><img src=\"images/btn_first.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
			sbRet.append(
					"<a class=\"prev\" href=\"#none\" title=\"Previous Page\"><img src=\"images/btn_prev.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
		}
		...
		for (int i = 0; i < pageCount && i < pages.length; i++) {
			if (pages[i][1].equals("-1")) {
				...
				sbRet.append("<a href=\"#none\" onClick=\"javascript:doPaging('" + pages[i][1]
						+ "');\" title=\"Paging\"> " + pages[i][0] + " </a>");
			}
		}

		if (wnanchor.getBundleNext() != -1) {
			sbRet.append("<a class=\"next\" href=\"#none\" onClick=\"javascript:doPaging('" + wnanchor.getBundleNext()
					+ "')\" title=\"Next page\"><img src=\"images/btn_next.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
			sbRet.append("<a class=\"end\" href=\"#none\" onClick=\"javascript:doPaging('" + lastPageNum
					+ "')\" title=\"Last page\"><img src=\"images/btn_end.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
		} else {
			sbRet.append(
					"<a class=\"next\" href=\"#none\" title=\"Next page\"><img src=\"images/btn_next.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
			sbRet.append("<a class=\"end\" href=\"#none\" onClick=\"javascript:doPaging('" + lastPageNum
					+ "')\" title=\"Last page\"><img src=\"images/btn_end.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
		}
	 ...
	 * @return 페이징 문자열 반환
	 */
	public String getPageLinksWebAccess(String base, String url, int startCount, int totalCount, int viewListCount,
			int bundleCount) {
		...
		if (wnanchor.getFirstPage() != -1) {
			sbRet.append("<A HREF='"
					+ replaceURL(RequestURI, RequestURL, "startCount", String.valueOf(wnanchor.getFirstPage())) + "'  >"
					+ ppreImg + "</A>&nbsp;&nbsp;");
		} else {
		...
		for (int i = 0; i < pageCount && i < pages.length; i++) {
			if (pages[i][1].equals("-1")) {
				...
				sbRet.append("<A HREF='" + replaceURL(RequestURI, RequestURL, "startCount", String.valueOf(pages[i][1]))
						+ "' class='nav'> " + pages[i][0] + " </A>");
			}
			if (pageCount > i + 1) {
		...
		if (wnanchor.getBundleNext() != -1) {
			sbRet.append("&nbsp;&nbsp;<A HREF='"
					+ replaceURL(RequestURI, RequestURL, "startCount", String.valueOf(wnanchor.getBundleNext())) + "'>"
					+ nextImg + "</A>");
		} else {
		...
		if (wnanchor.getLastPage() != -1) {
			sbRet.append("&nbsp;&nbsp;<A HREF='"
					+ replaceURL(RequestURI, RequestURL, "startCount", String.valueOf(wnanchor.getLastPage())) + "'>"
					+ nnextImg + "</A>");
		} else {
	...
	*  @return 컬렉션 명
	*/
	public String getCollectionNameLanguage(String col, String languageCode) {
		for (int idx = 0; idx < COLLECTIONS.length; idx++) {
			String coll = wncol.COLLECTION_INFO[idx][COLLECTION_NAME].trim();
			if (coll.equals(col)) {
				String temp = wncol.COLLECTION_INFO[idx][COLLECTION_KOR];
