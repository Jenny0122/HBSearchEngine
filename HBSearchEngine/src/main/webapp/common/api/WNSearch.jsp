...
include file="./WNCommon.jsp" %><%!
	...
	public class WNSearch {
		...
		public int search(String query, boolean isRealQuery) {
		    return search(query, isRealQuery, null, HI_SUM_ONON, COMMON_OR_WHEN_NORESULT_OFF, CONNECTION_CLOSE, false, "appr");
		}
		...
		public int search(String query, boolean isRealQuery, int connectionOpt, boolean useSuggestedQuery) {
		    ...
		    return search(query, isRealQuery, null, HI_SUM_ONON, COMMON_OR_WHEN_NORESULT_OFF, connectionOpt, useSuggestedQuery, "appr");
		}
		 ...
		 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
		 */
		public int search(String query, boolean isRealQuery, int connectionOpt, boolean useSuggestedQuery, String[] userInfo, String apprType) {
		    this.connectionOpt = connectionOpt;
		    return search(query, isRealQuery, userInfo, HI_SUM_ONON, COMMON_OR_WHEN_NORESULT_OFF, connectionOpt, useSuggestedQuery, apprType);
		}
		 ...
		 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
		 */
		private int search(String query, boolean isRealQuery, String[] userInfo, int hiSum, int useOrResult, int connectionOpt, boolean useSuggestedQuery, String apprType) {
			this.connectionOpt = connectionOpt;
		  	this.hiSum = hiSum;
		  	int ret = 0;
		  	if(!isUidSrch) {
		...
		public int setCollectionBasicInfo(int idx, String apprType) {
			...
			if(analyzer.length < 5 || pageInfo.length < 2 
				...
				appendWarnings("[WARNING] [setCollectionBasicInfo] ["+ THIS_COLLECTIONS[idx] 
			...
			if(checkKeywordAnalyzer(THIS_COLLECTIONS[idx], useKma, isCase, useOriginal, useSynonym, useDuplicateDetection)== -1) {
				return -1;
			}
			
			ret = addCollection(idx, useKma, isCase, useOriginal, useSynonym, useDuplicateDetection, apprType);

			if(checkSetPageInfo(THIS_COLLECTIONS[idx], this.hiSum, Integer.parseInt(pageInfo[0]), Integer.parseInt(pageInfo[1])) == -1 ) {
				return -1;
			}
			ret = common.setPageInfo(THIS_COLLECTIONS[idx], this.hiSum,
			        Integer.parseInt(pageInfo[0]), Integer.parseInt(pageInfo[1]));
				
		    ret = common.setRanking(THIS_COLLECTIONS[idx], rankingOption[0], rankingOption[1],  parseInt(rankingOption[2], 100));
		 ...
		 * @return 성공이면 0을 반환한다. 실패면 0이 아닌 값을 반환한다.
		 */
		public int addCollection(int idx, int useKma, int isCase, int useOriginal, int useSynonym, int useDuplicateDetection, String apprType){
			int ret = 0;
			String[] THIS_COLLECTIONS = null;
		    if (apprType.equals("mig")){
		    	THIS_COLLECTIONS = COLLECTIONS_MIG;
		...
		public int setSearchFieldInfo(int idx, String apprType) {
			...
			if(searchFieldList.length < 1) {
				appendWarnings("[WARNING] [w3AddSearchField] [setAllSearchFieldInfo] [" + THIS_COLLECTIONS[idx] + "] SEARCH_FIELD should be defined.");
				return -1;
			}
			int i = checkFieldName("[w3AddSearchField]", THIS_COLLECTIONS[idx], searchFieldList, SEARCH_FIELD);
			if( i != 0) {
				appendWarnings("[WARNING] [w3AddSearchField] [setAllSearchFieldInfo] [" + THIS_COLLECTIONS[idx] + "] '" + searchFieldList[i-1] + "' is not exist.");
				return -1;
			}
		...
		public int setCollectionQuery(int idx, String apprType) {
			...
			if (!wncol.COLLECTION_INFO[idx][COLLECTION_QUERY].equals("")) {
				if(isDebug) {
					checkCollectionQueryFieldName("[w3SetCollectionQuery] [setCollectionQuery]", THIS_COLLECTIONS[idx],wncol.COLLECTION_INFO[idx][COLLECTION_QUERY] );
				}
				ret = common.setCollectionQuery(THIS_COLLECTIONS[idx],
		...
		public int setDateRange(int idx, String apprType) {
			...
			if (!wncol.COLLECTION_INFO[idx][DATE_RANGE].equals("")) {
				...
				if(dateFields.length < 2) {
					appendWarnings("[WARNING] [w3SetDateRange] [setDateRange] [" + THIS_COLLECTIONS[idx] + "] DATE_RANGE : '" + wncol.COLLECTION_INFO[idx][DATE_RANGE] + "' error.");
					return -1;
				}
		...
		public int setRankRange(int idx, String apprType) {
			...
			if (!wncol.COLLECTION_INFO[idx][RANK_RANGE].equals("")) {
				...
				if(rankFields.length < 2) {
					appendWarnings("[WARNING] [w3SetRankRange] [setRankRange] [" + THIS_COLLECTIONS[idx] + "] RANK_RANGE : '" + wncol.COLLECTION_INFO[idx][RANK_RANGE] + "' error.");
					return -1;
				}
		...
		public int setPrefixQuery(int idx, String apprType){
			...
			if (!wncol.COLLECTION_INFO[idx][EXQUERY_FIELD].equals("")) {
				...
				ret = common.setPrefixQuery(THIS_COLLECTIONS[idx],
				                wncol.COLLECTION_INFO[idx][EXQUERY_FIELD]);
			}
			return ret;
		}
		...
		public int setBoostCategory(int idx, String apprType){
		    ...
		    if (!wncol.COLLECTION_INFO[idx][CATEGORY_BOOST].equals("")) {
				...
				if(dataFields.length == 3){
					ret = common.setBoostCategory(THIS_COLLECTIONS[idx],
		...
		public int setAuthorityQuery(int idx, String apprType) {
			...
			if (!wncol.COLLECTION_INFO[idx][AUTH_QUERY].equals("")) {
				...
				if(dataFields.length == 4){
					ret = common.setAuthorityQuery(THIS_COLLECTIONS[idx],
		...
		*/
		...
		String[] THIS_COLLECTIONS = null;
			...
			if (!wncol.COLLECTION_INFO[idx][PROPERTY_GROUP].equals("")) {
				...
				if(dataFields.length == 4){
					ret = common.setPropertyGroup(THIS_COLLECTIONS[idx],
		...
		*/
			...
			if (!wncol.COLLECTION_INFO[idx][GROUP_BY].equals("")) {
				...
				// 그룹화할 필드가 fastacces로 잡혀있는지 체크
					if(checkValidField(THIS_COLLECTIONS[idx], groupby[0], FAST_ACCESS) == -1) {
						appendWarnings("[WARNING] [w3SetGroupBy] [setGroupBy] [" + THIS_COLLECTIONS[idx] + "] " +groupby[0]+ " is not fastaccess field.");
					}
				}
				//그룹전달인자 확인
				if(groupby.length == 2){
					...
					appendWarnings("[WARNING] [w3SetGroupBy] [setGroupBy] [" + THIS_COLLECTIONS[idx] + "] " +wncol.COLLECTION_INFO[idx][GROUP_BY]+ "  Check variable format and value.");
				}
			}
			return ret;
		}
		...
		public int setResultFieldInfo(int idx, String apprType) {
			...
			if(resultFieldList.length < 1) {
				appendWarnings("[WARNING] [w3AddDocumentField] [setResultFieldInfo] [" + THIS_COLLECTIONS[idx] + "] RESULT_FIELD should be defined.");
				return -1;
			}
			int i = checkFieldName("[w3AddDocumentField]", THIS_COLLECTIONS[idx], resultFieldList, RESULT_FIELD);
			if( i != 0 ) {
				appendWarnings("[WARNING] [w3AddDocumentField] [setResultFieldInfo] [" + THIS_COLLECTIONS[idx] + "] '" + resultFieldList[i-1] + "' is not exist.");
				return -1;
			}
		...
		*/
		...
		public String getPageLinks(int startCount, int totalCount, int viewListCount, int bundleCount) {
			...
			if(wnanchor.getFirstPage() != -1) {
				sbRet.append("<a class=\"first\" href=\"#none\" onClick=\"javascript:doPaging('0');\" title=\"First Page\"><img src=\"images/btn_first.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
				sbRet.append("<a class=\"prev\" href=\"#none\" onClick=\"javascript:doPaging('"+wnanchor.getBundleBefore()+"');\" title=\"Previous Page\"><img src=\"images/btn_prev.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
			} else {
				sbRet.append("<a class=\"first\" href=\"#none\" onClick=\"javascript:doPaging('0');\" title=\"First Page\"><img src=\"images/btn_first.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
				sbRet.append("<a class=\"prev\" href=\"#none\" title=\"Previous Page\"><img src=\"images/btn_prev.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
			} 
			...
			if(wnanchor.getBundleNext() != -1) {
    			sbRet.append("<a class=\"next\" href=\"#none\" onClick=\"javascript:doPaging('"+wnanchor.getBundleNext()+"')\" title=\"Next page\"><img src=\"images/btn_next.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
    			sbRet.append("<a class=\"end\" href=\"#none\" onClick=\"javascript:doPaging('"+lastPageNum+"')\" title=\"Last page\"><img src=\"images/btn_end.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
			} else {
				sbRet.append("<a class=\"next\" href=\"#none\" title=\"Next page\"><img src=\"images/btn_next.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
    			sbRet.append("<a class=\"end\" href=\"#none\" onClick=\"javascript:doPaging('"+lastPageNum+"')\" title=\"Last page\"><img src=\"images/btn_end.gif\" width=\"25\" height=\"25\" alt=\"\" /></a>");
			}
