...
function drawArk() {
	...
	// 툴팁 이미지 생성
	//str += "<div id=\"tooltip01\" style=\"position:absolute; left:330px !important; display:none; cursor:pointer; z-index:999999;\"><img id=\"tooltipIcon01\" src=\"" + arkPath +"/img/tooltip_01.gif\"/></div>";

	// 자동완성 결과 생성
	str += "<div class='auto' id=\"" + wrapId +"\">";
	str += "	<ul class='recomm' id=\"" + contentListId + "\">";
	str += "	</ul>";
	str += "	<div id=\"" + footerId +"\">";
	str += "</div>";
			...
			$.each(data.result, function(i, result) {
				...
				if (totalCount > 0) {
					...
					$.each(result.items, function(num,item){
						...
						var tmp = item.keyword;
						tmp = tmp.replace(query, "<strong class='tx_key'>" + query + "</strong>");

						str += "<li id=\"bg" + num + "\" onclick=\"onClickKeyword(" + num + ");\" onmouseover=\"onMouseOverKeyword(" + num + ");\"";
						str += " 	onmouseout=\"onMouseOutKeyword(" + num + ");\">" + tmp;
						str += " 	<span id=\"f" + num + "\" style=display:none;>" + item.keyword + "</span>";
						str += "</li>";
					});
				}
...
function setArkFooter() {
	...
	if (isArk) {
		...
		str += "	<strong>자동 추천 기능</strong><br>검색어 입력시 자동으로 관련어를 추천합니다.</strong>";
		str += "</div>";
		
		str += "<div class='auto_btn_wrap'>";
		str += "	<a id=\"arkOn\" href='#' class='auto_btn' onClick=\"setArkOn();\">기능켜기</a>";
		str += "</div>";
